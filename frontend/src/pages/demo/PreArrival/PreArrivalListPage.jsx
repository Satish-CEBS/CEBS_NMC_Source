...
const PreArrivalListPage = () => {
    ...
    const applyFilters = () => {
        return data.filter((item) => {
            ...
            const fromDateMatch = filters.fromDate ? itemDate >= new Date(filters.fromDate) : true;
            const toDateMatch = filters.toDate ? itemDate <= new Date(filters.toDate) : true;
    ...
    const sortData = (sortedData) => {
        ...
        return [...sortedData].sort((a, b) => {
            const aVal = (a.vessel?.[key] || a.voyage?.[key] || a[key] || '').toString().toLowerCase();
            const bVal = (b.vessel?.[key] || b.voyage?.[key] || b[key] || '').toString().toLowerCase();
            ...
            <div className="dashboard-body">
                ...
                <main className="dashboard-content">
                    ...
                    <div className="filter-bar">
                        <input name="vesselName" placeholder="Vessel Name" onChange={handleFilterChange} />
                        <input name="imo" placeholder="IMO" onChange={handleFilterChange} />
                        <input name="voyageNo" placeholder="Voyage No" onChange={handleFilterChange} />
                        <input name="submittedBy" placeholder="Submitted By" onChange={handleFilterChange} />
                        <input name="lastPort" placeholder="Last Port" onChange={handleFilterChange} />
                            ...
                            <tbody>
                                {paginatedData.map((entry) => (
                                    <tr key={entry.pre_arrival_id}>
                                        ...
                                        <td>{entry.voyage?.eta ? new Date(entry.voyage.eta).toLocaleString() : ''}</td>
                                        <td><span className={getBadgeClass(entry.status)}>{entry.status}</span></td>
                                        <td>{entry.submitted_by}</td>
                                        <td>{entry.submitted_date ? new Date(entry.submitted_date).toLocaleString() : ''}</td>
                                        <td>
                                            <button
                                                className="view-btn"
                                                onClick={() => window.location.href = `/prearrival-view/${entry.pre_arrival_id}`}>
                                                View
                                            </button>
                                        </td>
                                    </tr>
