...
const etaData = [
    ...
    { day: 'Wed', count: 9 }, { day: 'Thu', count: 15 },
    { day: 'Fri', count: 21 }, { day: 'Sat', count: 6 }, { day: 'Sun', count: 11 }
];
...
const PreArrivalDashboard = () => {
    ...
    return (
        <div className="dashboard">
            <InnerHeader />
            <InnerSubHeader title="Pre-Arrival Operations Dashboard" />
            <div className="dashboard-body">
                <SidebarMenu />
                <main className="dashboard-content">
                    <div className="dashboard-tabs">
                        <button className={tab === 'overview' ? 'active' : ''} onClick={() => setTab('overview')}>Overview</button>
                        <button className={tab === 'geo' ? 'active' : ''} onClick={() => setTab('geo')}>Geo Maps</button>
                        <button className={tab === 'alerts' ? 'active' : ''} onClick={() => setTab('alerts')}>Risk & Alerts</button>
                        <button onClick={() => exportChart(overviewRef, 'PreArrivalDashboard')}><DownloadIcon /> Export PDF</button>
                    </div>
                        ...
                        <div className="cards-container" ref={overviewRef}>
                            ...
                            <div className="chart-card" ref={etaRef}>
                                <div className="card-header">
                                    ...
                                    <button onClick={() => exportChart(etaRef, 'eta-distribution')}><DownloadIcon fontSize="small" /></button>
                                </div>
                                <ResponsiveContainer width="100%" height={250}>
                                    <BarChart data={etaData}>
                                        ...
                                        <YAxis />
                                        <Tooltip />
                                        <Bar dataKey="count" fill="#ff69b4" onClick={(d) => handleDrilldown(d.day)} />
                                    </BarChart>
                                </ResponsiveContainer>
                                <p className="chart-description">Shows daily expected vessel arrivals for berth planning.</p>
                            </div>
                            ...
                            <div className="chart-card" ref={flagRef}>
                                <div className="card-header">
                                    <h3>Flag States</h3>
                                    <button onClick={() => exportChart(flagRef, 'flag-states')}><DownloadIcon fontSize="small" /></button>
                                </div>
                                <ResponsiveContainer width="100%" height={250}>
                                    <PieChart>
                                        <Pie
                                ...
                                <p className="chart-description">Represents vessel counts grouped by flag state.</p>
                            </div>
                            ...
                            <div className="chart-card" ref={checklistRef}>
                                <div className="card-header">
                                    ...
                                    <button onClick={() => exportChart(checklistRef, 'checklist-compliance')}><DownloadIcon fontSize="small" /></button>
                                </div>
                                <ResponsiveContainer width="100%" height={250}>
                                    <PieChart>
                                        <Pie data={checklistData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label>
                                ...
                                <p className="chart-description">Tracks how many vessels submitted all required documentation.</p>
                            </div>
                            ...
                            <div className="chart-card full-width">
                                ...
                                <ResponsiveContainer width="100%" height={250}>
                                    <LineChart data={liveETA}>
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <Line type="monotone" dataKey="arrivals" stroke="#8a2be2" activeDot={{ r: 8 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                                <p className="chart-description">Displays expected hourly arrivals to prepare gates and support.</p>
                            </div>
                        </div>
                    )}
                    ...
                    {tab === 'geo' && (
                        <div className="map-card">
                            ...
                            <MapContainer center={[22, 35]} zoom={2} scrollWheelZoom={false} className="leaflet-map">
                                <TileLayer
                            ...
                            <p className="chart-description">Visual map of vessels based on port of origin and flag state.</p>
                        </div>
                    )}
                    ...
                    {tab === 'alerts' && (
                        <div className="cards-container">
                            ...
                            <div className="alert-card orange">
                                ...
                                <p>1 vessel under close monitoring due to compliance history.</p>
                            </div>
                        </div>
                    )}
                </main>
