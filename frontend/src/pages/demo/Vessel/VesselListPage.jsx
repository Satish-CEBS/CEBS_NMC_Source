...
const VesselListPage = () => {
    ...
    useEffect(() => {
        let result = vessels.filter(v => {
            return (
                ...
                (!filters.ship_flag_code_id || v.ship_flag_code_id?.toLowerCase().includes(filters.ship_flag_code_id.toLowerCase())) &&
                (!filters.vessel_id || v.vessel_id?.toString().includes(filters.vessel_id)) &&
    ...
    const vesselTypes = [...new Set(vessels.map(v => v.ship_type_ais).filter(Boolean))];
    const vesselStatuses = [...new Set(vessels.map(v => v.service_status).filter(Boolean))];

    const indexOfLastRecord = currentPage * recordsPerPage;
    const indexOfFirstRecord = indexOfLastRecord - recordsPerPage;
    const currentRecords = filteredVessels.slice(indexOfFirstRecord, indexOfLastRecord);
            ...
            <div className="vessel-list-content">
                ...
                <div className="inner-content">
                    <div className="page-header">
                        ...
                        <button className="create-button">Create New Pre-arrival Notification</button>
                    </div>

                    {/* Filter row */}
                    <div className="filter-row">
                        <input type="text" name="name" placeholder="Vessel Name" onChange={handleInputChange} />
                        <input type="text" name="imo_no" placeholder="IMO Number" onChange={handleInputChange} />
                        <input type="text" name="mmsi_no" placeholder="MMSI Number" onChange={handleInputChange} />
                        <input type="text" name="ship_flag_code_id" placeholder="Flag Code" onChange={handleInputChange} />
                        <input type="text" name="vessel_id" placeholder="Vessel ID" onChange={handleInputChange} />
                    ...
                    <table className="vessel-table">
                        ...
                        <tbody>
                            {currentRecords.length > 0 ? (
                                currentRecords.map((v, idx) => (
                                    <tr key={v.vessel_id || idx}>
                                        ...
                                        <td>
                                            <button
                                                className="view-btn"
                                                onClick={() => navigate(`/vessel/view/${v.vessel_id}`)}
                                            >
                                                View
                                            </button>
                                        </td>
                                ...
                                <tr><td colSpan="8">No vessels found.</td></tr>
                            )}
                        </tbody>
                    </table>
